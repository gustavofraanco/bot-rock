...
class Partida:
    ...
    async def _anunciar_eliminados(self, resposta, eliminados, ninguem_acertou=False):
        if ninguem_acertou:
            desc = (f"# <:fale_finalizada:1488692025984553241> Rodada anulada!\n"
                    ...
                    f"<:dale_atencao:1478412503036989480> **Ninguém acertou!** A rodada será reiniciada com uma nova imagem.")
        else:
            for j in eliminados:
            ...
            desc = (f"# <:fale_finalizada:1488692025984553241> Rodada finalizada!\n"
                    f"* A resposta era `{resposta}`\n"
                    f"Jogador(es) eliminado(os):\n{mencoes}")
        
        embed = discord.Embed(description=desc, color=0xF1C40F)
        icon = "https://images-ext-1.discordapp.net/external/PZRe1YDxbibtfjepaLXCwL4f_tceKC7mPAON8xo-KQk/%3Fsize%3D2048/https/cdn.discordapp.com/emojis/1488693040636891235.png?format=webp"
        embed.set_footer(text=f"Restam {len(self.jogadores_ativos)} jogadores", icon_url=icon)
        await self.canal.send(embed=embed)
    ...
    async def _enviar_embed_pergunta(self, pergunta: dict, tempo: int):
        ...
        embed = discord.Embed(
            description=(f"# <:dale_info:1478237600908054548> ACERTE A IMAGEM\nㅤ\n"
    ...
    async def _encerrar(self):
        ...
        if self.vencedor:
            embed = discord.Embed(
                description=(f"# <:fale_vencedor:1488653915464663060> Resta apenas 1!\n"
